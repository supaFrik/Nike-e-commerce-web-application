                    ...
                    <div class="header-links">
                        ...
                        <c:choose>
                            <c:when test="${isAuthenticated}">
                                ...
                                <c:if test="${isAdmin}">
                                    <span class="separator">|</span>
                                    <a href="${env}/admin" class="header-link header-switch-link" aria-label="Switch to admin suite">
                                        Admin
                                    </a>
                                </c:if>
                            ...
                            <c:otherwise>
                              <a href="${env}/login" class="header-link">Tham gia</a>
                                                            <span class="separator">|</span>
                              <a href="${env}/login" class="header-link">Đăng nhập</a>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </div>
    ...
    <nav class="navbar">
            ...
            <div class="nav-content">
                ...
                <div class="mobile-actions">
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a href="${env}/login" class="icon-btn mobile-profile-btn" aria-label="Login">
                                <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                                    <circle cx="12" cy="7" r="4"></circle>
                                </svg>
        ...
        <div class="mobile-menu-overlay" id="mobileMenuOverlay">
            <div class="mobile-menu-content">
                <button class="mobile-menu-close" type="button" data-mobile-menu-toggle aria-label="Close menu">
                    ✕
                </button>
                    ...
                    <c:choose>
                        <c:when test="${isAuthenticated}">
                            <div class="mobile-member-actions">
                                <a href="${env}/profile" class="mobile-member-btn primary">Xin chào, <c:out value="${displayName}" /></a>
                                <c:if test="${isAdmin}">
                                    <a href="${env}/admin" class="mobile-member-btn secondary">Admin Suite</a>
                                </c:if>
                                <form action="${env}/logout" method="post" style="display:inline;">
                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
                                    <button type="submit" class="mobile-member-btn secondary" style="border:none;cursor:pointer;">Đăng xuất</button>
                                </form>
                            </div>
                        </c:when>
                        <c:otherwise>
                            <div class="mobile-member-actions">
                                <a href="${env}/login" class="mobile-member-btn primary">Tham gia</a>
                                <a href="${env}/login" class="mobile-member-btn secondary">Đăng nhập</a>
                            </div>
                        </c:otherwise>
                    </c:choose>
                </div>
