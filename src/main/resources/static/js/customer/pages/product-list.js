...
(function(){
  const state = { items: [], page: 1, pageSize: 20, totalItems: 0, totalPages: 1, sort: 'name', category: 'all', search: '' };
  const els = {};
  ...
  function resolveImageUrl(imageUrl){
    ...
    if(/^\/https?:\/\//i.test(normalized)) return normalized.slice(1);
    if(/^https?:\/\//i.test(normalized)) return normalized;
    return normalized.startsWith('/') ? ctx()+normalized : `${ctx()}/${normalized}`;
  }
  ...
  function dynamicCategoriesIntoTabs(){ if(!window.__CATEGORIES__ || !els.categoryTabs) return; window.__CATEGORIES__.forEach(c=>{ const key=c.key; if(!els.categoryTabs.querySelector(`[data-filter="${key}"]`)){ const btn=document.createElement('button'); btn.className='tab'; btn.type='button'; btn.dataset.filter=key; btn.role='tab'; btn.textContent=c.name; btn.setAttribute('aria-selected','false'); els.categoryTabs.appendChild(btn); } }); }
  ...
  function fetchProducts(){ if(abortCtrl) abortCtrl.abort(); abortCtrl = new AbortController(); setBusy(true); fetch(ctx() + '/admin/api/products?' + buildQuery(), { signal: abortCtrl.signal }).then(r=> r.ok? r.json(): Promise.reject(r)).then(applyResponse).catch(err=>{ if(err.name==='AbortError') return; console.error('Fetch products failed', err); showEmpty('Failed to load products'); }).finally(()=> setBusy(false)); }
  ...
  function buildCard(p){ const template = els.cardTemplate?.content?.firstElementChild; const node = template? template.cloneNode(true): document.createElement('article'); node.classList.add('product-card'); const thumb=node.querySelector('[data-ref="thumb"]')||node.querySelector('.thumb'); const img=node.querySelector('[data-ref="image"]'); const imageUrl = resolveImageUrl(p.imageUrl); const hasImage = !!imageUrl; if(img){ if(hasImage){ img.src = imageUrl; img.alt = p.name? (p.name+' image'): 'Product image'; img.onerror=()=>{ if(thumb){ thumb.classList.add('no-image'); } img.remove(); }; } else { if(thumb){ thumb.classList.add('no-image'); } img.remove(); } }
    const title=node.querySelector('[data-ref="title"]'); if(title) title.textContent = p.name||'Unnamed'; const price=node.querySelector('[data-ref="price"]'); if(price) price.textContent = formatCurrency(p.displayPrice); const stock=node.querySelector('[data-ref="stock"]'); if(stock) stock.textContent = (p.stock!=null? p.stock+' in stock':'\u2014'); const statusDot = node.querySelector('[data-ref="statusDot"]'); let statusVal = p.productStatus || p.status || 'UNKNOWN'; if(statusDot){ statusDot.dataset.status = statusVal; statusDot.title = 'Status: ' + statusVal.replace(/_/g,' '); } node.dataset.status = statusVal; node.tabIndex=0; node.addEventListener('click', ()=> openEditor(p)); node.addEventListener('keydown', e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); openEditor(p); }}); return node; }
  function renderPagination(){ if(!els.paginationList) return; els.paginationList.innerHTML=''; for(let i=1;i<=state.totalPages;i++){ const btn=document.createElement('button'); btn.type='button'; btn.textContent=i; btn.dataset.page=i; btn.className='page-pill'; if(i===state.page) btn.classList.add('active'); els.paginationList.appendChild(btn); } if(els.paginationSummary){ els.paginationSummary.textContent = `${state.totalItems} product${state.totalItems!==1?'s':''} • Page ${state.page} of ${state.totalPages}`; } }
  ...
  function openEditor(p){ if(!els.editPanel) return; els.editPanel.classList.add('open'); if(els.editImage) els.editImage.src = resolveImageUrl(p.imageUrl) || `${ctx()}/images/placeholder.svg`; if(els.editForm){ els.editForm.name.value = p.name||''; els.editForm.description.value = p.description||''; els.editForm.price.value = p.displayPrice!=null? p.displayPrice:0; els.editForm.stock.value = p.stock!=null? p.stock:0; ensureCategoryOption(p.categoryKey, p.category); if(els.editCategorySelect && p.categoryKey) els.editCategorySelect.value=p.categoryKey; els.editForm.dataset.id = p.id; }
    const fullLink = document.getElementById('fullEditLink');
    if(fullLink){
      if(p && p.id != null){
        fullLink.href = ctx() + '/admin/product/edit/' + encodeURIComponent(p.id);
        fullLink.removeAttribute('aria-disabled');
        fullLink.classList.remove('disabled');
      } else {
        fullLink.href = '#';
        fullLink.setAttribute('aria-disabled','true');
        if(!fullLink.classList.contains('disabled')) fullLink.classList.add('disabled');
      }
    }
  }
  function closeEditor(){ els.editPanel?.classList.remove('open'); const fullLink=document.getElementById('fullEditLink'); if(fullLink){ fullLink.href='#'; fullLink.setAttribute('aria-disabled','true'); if(!fullLink.classList.contains('disabled')) fullLink.classList.add('disabled'); } }
  ...
  function putUpdate(id, body){ const {token,header}=csrf(); fetch(ctx()+'/admin/api/products/'+id,{ method:'PUT', headers:{ 'Content-Type':'application/json', ...(token&&header? {[header]:token}: {}) }, body:JSON.stringify(body) }).then(r=> r.ok? r.json(): r.json().then(j=>Promise.reject(j))).then(()=>{ closeEditor(); fetchProducts(); }).catch(err=>{ console.error('Update failed', err); alert('Update failed: '+ (err.error||'Error')); }); }
  ...
  function deleteProduct(id){ const {token,header}=csrf(); fetch(ctx()+'/admin/api/products/'+id,{ method:'DELETE', headers:{ ...(token&&header? {[header]:token}: {}) } }).then(r=> r.ok? r.json(): r.json().then(j=>Promise.reject(j))).then(()=>{ closeEditor(); state.items = state.items.filter(p=> p.id != id); renderGrid(); fetchProducts(); }).catch(err=>{ console.error('Delete failed', err); alert('Delete failed: '+ (err.error||'Error')); }); }
